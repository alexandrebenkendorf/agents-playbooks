...
### Use Type Guards to Eliminate Redundant Checks

When you have a type guard function, use it to both narrow types AND validate, avoiding redundant runtime checks.

```ts
// Type guard function
function isTrackedElement(el: unknown): el is TrackedElement {
...
function handleFocus(target: EventTarget | null) {
  ...
  trackElement(target); // target is TrackedElement
}

// ✅ Do - Single type guard handles both validation and type narrowing
function handleFocus(target: EventTarget | null) {
  if (!isTrackedElement(target)) {
    return;
  }

  // target is now TrackedElement, all checks done
  trackElement(target);
}
```
...
**Note:** If you need additional filtering beyond the type guard (e.g., only form elements, not buttons), combine checks efficiently:

```ts
// ✅ Do - Combine type guard with additional validation
const FORM_ELEMENT_TAGS = ['INPUT', 'SELECT', 'TEXTAREA'];

function handleFocus(target: EventTarget | null) {
  // Type guard narrows AND validates tracked elements
  if (!isTrackedElement(target) || !FORM_ELEMENT_TAGS.includes(target.tagName)) {
    return;
  }

  // target is TrackedElement AND a form element
  trackElement(target);
}
```
...
### Always Export Utilities from Barrel Files

When creating utility functions, always add them to the module's `index.ts` barrel export for discoverability and consistent imports.

```ts
// ❌ Don't - Leave utilities unexported
// utils/isValidEmail.ts
...
// isValidEmail missing!

// ❌ Don't - Force consumers to use deep imports
import { isValidEmail } from '../utils/isValidEmail';

// ✅ Do - Export from barrel file
// utils/index.ts
export * from './formatDate';
export * from './parseJson';
export * from './isValidEmail';

// ✅ Do - Use barrel import
...
- Better discoverability (developers know where to import from)
- Consistent import style across codebase

**Rule:** Whenever you create a new utility file, immediately add it to `index.ts` exports.
