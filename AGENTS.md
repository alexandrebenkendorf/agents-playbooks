...
- Note: This repository is the source playbook, so skills live in `/skills/` and `.agents/` usually does **not** exist here.
- If `.agents/` exists in a consumer project, read `.agents/README.md` first (if present) and treat its listed skills as available.
- Then scan for skill folders under `.agents/local/skills/*/SKILL.md` and `.agents/agents-playbooks/skills/*/SKILL.md`.
- Also scan `/skills/*/SKILL.md` when working in this repository.
...
- DO vendor the skill **as-is** under `/skills/<category>/<skill-name>/`
