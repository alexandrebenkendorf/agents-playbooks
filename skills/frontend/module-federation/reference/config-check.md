...
```bash
node <skill-dir>/scripts/config-exposes-check.js --context '<MFContext-JSON>'
```
...
**CONFIG-ASYNC-ENTRY · warning — async entry not configured (maps to RUNTIME-005)**

- `experiments.asyncStartup = true` is not set in the **bundler** config (this is a bundler-level
  experiment, not an MF plugin option)
- Without it, shared dependencies become async modules and a synchronous entry throws
  `RUNTIME-005 · Invalid loadShareSync function call from bundler runtime`. The alternative fix is a
  manual `bootstrap.js` async entry, or `eager: true` on the shared deps.
- Exception: not required when using `@module-federation/modern-js-v3` or `@module-federation/modern-js`, which enable the matching async entry automatically (except in Modern.js build mode)
- Not applicable to Vite or Next.js — `experiments.asyncStartup` is a Webpack/Rspack-family option
- Note: Rspack requires version > 1.7.4 to support this option
- Reference: https://module-federation.io/blog/hoisted-runtime
- The check reads `bundler.experiments.asyncStartup` from MFContext. If MFContext reports it as
  `null` (could not be determined), say so instead of asserting it is missing.
