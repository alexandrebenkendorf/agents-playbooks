...
Read `package.json` to check which frameworks are present. Set singletons accordingly, and always
add the trailing-slash prefix key so subpath imports (`react-dom/client`, `react/jsx-runtime`) are
shared too:

- If `react` + `react-dom` present:
  ```ts
  shared: {
  ...
  ```
- If `vue` present: add `vue: { singleton: true }` and `'vue/': { singleton: true }`
...
> Note: every MF option except [`dev`](https://module-federation.io/configure/dev) is supported by the Vite plugin.
...
+import mfConfig from './module-federation.config';
 ...
 export default defineConfig({
+  // Producers must serve assets from an absolute origin so consumers resolve them correctly.
...
> If you need build targets below `chrome89`, add `vite-plugin-top-level-await`.

> Planning to use `dev.remoteHmr` between a Vite host and Vite remotes? `@vitejs/plugin-react`,
> `vite`, and `@module-federation/vite` must be pinned to the same versions on both sides. See
> `./perf.md` → "Remote HMR compatibility".
