...
- If the code is any other `RUNTIME-xxx` (the official list currently runs to `RUNTIME-015`), do **not** continue local diagnosis here. Instead, read and follow `./docs.md` and look up the corresponding runtime error code in `https://module-federation.io/guide/troubleshooting/runtime`
...
Before asking the user to paste logs manually, try to capture structured runtime evidence.

### 3a. Prefer the Observability Plugin when available

If the project already has `@module-federation/observability-plugin` (or the Divebell MF Extension)
enabled, read its report first — it gives structured Remote / Shared / Bridge evidence and
`diagnosis.facts` / `diagnosis.actions` without driving a browser. See
`https://module-federation.io/plugin/plugins/observability-plugin` and
`https://module-federation.io/guide/debug/divebell`.

Only fall back to raw CDP capture below when neither is available.

### 3b. Check whether Chrome remote debugging is available
...
- If not reachable: inform the user they need to launch Chrome with `--remote-debugging-port=9222` and proceed to Step 4 (fallback)

### 3c. Capture the failing page

Use the browser capture script against the failing page URL. Requires Node.js 21+ (built-in `WebSocket`).

Prefer this baseline command first:

```bash
node <skill-dir>/scripts/browser-capture.mjs "<failing-page-url>" 20000 --vars __FEDERATION__,__webpack_require__,window.__FEDERATION__
```

If the page is noisy or polling heavily, prefer:

```bash
node <skill-dir>/scripts/browser-capture.mjs "<failing-page-url>" 15000 --vars __FEDERATION__,window.__FEDERATION__ --no-entries
```

If the error only happens after user interaction, use the `--keep-tab` flag to capture a tab and then interact:

```bash
TAB=$(node <skill-dir>/scripts/browser-capture.mjs "<failing-page-url>" --keep-tab | jq -r .tabId)
node <skill-dir>/scripts/browser-capture.mjs --tab-id $TAB --click "<button text>" --vars __FEDERATION__ --close
```
